...
**Verbindlich ist dieses Repository.** Das Notebook klont es beim Start in die Colab-Sitzung und legt dort denselben Ordner `ergebnisse/` an; ein Google-Drive-Zugang wird weder für die Ausführung noch für die Bewertung benötigt. Zusätzlich liegt im Ergebnisordner `zweitbewertung_auswahl.csv` mit den vorab gezogenen Fällen für die Zweitbewertung.
...
Der Schalter `ABLAGE` bestimmt den Arbeitsordner: `"github"` klont dieses Repository in die Colab-Sitzung und packt am Ende jedes Laufs ein Ergebnispaket zum Herunterladen, `"drive"` arbeitet in Google Drive, `"colab"` nur in der flüchtigen Sitzung. Für die Bewertung ist gleichgültig, welcher Weg gewählt wurde; im Repository landet in jedem Fall derselbe Ergebnisordner.
